...
export class SedraDialog extends BaseElement{
	...
	static get styles(){
		return [ScrollbarStyle, SpinnerStyle, css`
			:host{
				...
				position:var(--sedra-dialog-position, absolute);
				top:0px;
				left:0px;
				width:100%;
				height:100%;
				background-color:var(--sedra-dialog-bg-color);
			...
			.container{
				box-sizing:border-box;
				width:100%;
				height:var(--sedra-dialog-container-height, calc(100% - 10px));
				background-color:var(--flow-background-color, #F00);
				z-index:1;
				border:var(--sedra-dialog-container-border, 2px solid var(--flow-primary-color));
				border-radius:var(--sedra-dialog-container-border-radius, 3px);
				max-width:var(--sedra-dialog-container-max-width, 700px);
				max-height:var(--sedra-dialog-container-max-height, 300px);
				margin:var(--sedra-dialog-container-margin, 5px auto);
				padding:var(--sedra-dialog-container-padding, 0px);
				position:relative;
			...
			:host([hideable]) .close-btn{
				display:var(--sedra-dialog-container-close-btn-display, inline-block)
			}
			.heading{
			...
			[hidden]{display:none}
			.buttons{margin:var(--sedra-dialog-buttons-margin, 10px auto);display:flex;width:var(--sedra-dialog-buttons-width,90%);}
	...
	pushHistory(uid=""){
		...
		name = name.replace(/\-/g, "");
		let key = name.toLowerCase().replace(/(sedra|dialog|mobile)/g, '')
