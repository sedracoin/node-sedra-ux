...
import {
	Deferred, GetTS, SDR, formatForMachine, formatForHuman,
...
import {initSedraFramework, Wallet, workerLog} from '@sedracoin/wallet-worker';
export {Wallet};
Wallet.setWorkerLogLevel(localStorage.walletWorkerLogLevel || 'none')

export {html, css, FlowFormat, dpc, baseUrl, debug};

export class SedraWalletUI extends BaseElement{
	...
	async initNetworkSettings() {
		...
		//const { network, port } = this.local_sedrad_settings;
			...
			<div class="tx-notifications">
				${notifications.map(n=>{
					return html`<div class="tx-notification">
						${n.compoundUTXOs?
							T(`Compounding UTXOs...`):
							i18n.t(`Preparing transaction for [n] SDR ....`)
							.replace('[n]', this.formatSDR(n.amount))}
					</div>`
				})}
				...
				return html`
					<flow-expandable class="tx-row" static-icon expand ?txin=${tx.in} ?txmoved=${tx.isMoved} ?txout=${!tx.in}
						...
						<div class="tx-title" slot="title">
							...
							<div class="amount">
								${tx.in?'':'-'}${this.formatSDR(tx.amount)} SDR
							</div>
						</div>
						...
						<div class="tx-body">
							${tx.note}
							<div class="tx-id">
								<a target="_blank" href="https://explorer.sedracoin.com/txs/${tx.id}">${tx.id}</a>
							</div>
							<div class="tx-address">
								${tx.myAddress?T('COMPOUNDING WALLET => '):''}
								<a target="_blank" href="https://explorer.sedracoin.com/addresses/${tx.address}">${tx.address}</a>
							</div>
						</div>
					</flow-expandable>
				`
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?txin=${tx.in} tx-version=${tx.version} ?txmoved=${tx.isMoved} ?txout=${!tx.in}>
						...
						<div class="tx-amount">${tx.in?'':'-'}${SDR(tx.amount)} SDR</div>
						<div class="br tx-note">${tx.note}</div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.sedracoin.com/txs/${tx.id.split(":")[0]}">${tx.id.split(":")[0]}</a>
						</div>
						<div class="tx-address">
							${tx.myAddress?T('COMPOUNDING WALLET => '):''}
							<a target="_blank" href="https://explorer.sedracoin.com/addresses/${tx.address}">${tx.address}</a>
		...
		items.map((tx, i)=>{
			...
			fields.forEach(field=>{
				switch(field){
					...
					break;
					case 'amount':
						row.push(escape(`${tx.in?'':'-'}${SDR(tx.amount)}`))
					break;
					case 'direction':
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?iscoinbase=${!tx.isCoinbase}>
						...
						<div class="tx-amount">${SDR(tx.satoshis)} SDR</div>
						<div class="br tx-mass"></div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.sedracoin.com/txs/${tx.txId}">${tx.id}</a>
						</div>
						<div class="tx-address">
							<a target="_blank" href="https://explorer.sedracoin.com/addresses/${tx.address}">${tx.address}</a>
						</div>
	...
	async alertUTXOIndexSupportIssue(){
		...
		let body = html`${i18nHTMLFormat(`'utxoindex' flag is missing from SEDRAD config.<br />
	...
	connectedCallback(){
		...
		let openDialog = document.createElement('sedra-open-dialog');
		openDialog.hideLogo = !!this.hideOpenWalletLogo;
		this.parentNode.insertBefore(openDialog, this.nextSibling)
		this.sendDialog = document.createElement("sedra-send-dialog"+mobileSuffix);
		this.parentNode.appendChild(this.sendDialog);
		this.receiveDialog = document.createElement("sedra-receive-dialog"+mobileSuffix);
		this.parentNode.appendChild(this.receiveDialog);
		this.seedsDialog = document.createElement("sedra-seeds-dialog");
		this.parentNode.appendChild(this.seedsDialog);
		let t9Dialog = document.createElement("sedra-t9-dialog");
		this.parentNode.appendChild(t9Dialog);
		let qrscannerDialog = document.createElement("sedra-qrscanner-dialog");
		qrscannerDialog.debug = this.debugscanner
		this.parentNode.appendChild(qrscannerDialog);
		let uploadFileDialog = document.createElement("sedra-upload-file-dialog");
		this.parentNode.appendChild(uploadFileDialog);

		const {workerCorePath} = window.SedraConfig||{}
		initSedraFramework({
			workerPath: workerCorePath||"/sedra-wallet-worker/worker.js?ident="+(window.SedraConfig?.ident||"")
		}).then(()=>{
			let encryptedMnemonic = getLocalWallet()?.mnemonic;
	...
	async handleInitDialogCallback({dialog, password, seedPhrase, encryptedMnemonic}){
		...
		if(!rpc)
			return FlowDialog.alert(i18n.t("Error"), i18n.t("Sedra Daemon config is missing."));
	...
	showTxDialog(){
		if(!this.txDialog){
			this.txDialog = document.createElement("sedra-tx-dialog");
	...
	async isValidAddress(address){
		...
		if(window.mobileMode && prefix=="sedratest")
			return true;
	...
	*/
	...
	async getSedraFromFaucet(amount) {
	...
	requestFaucetFunds() {
		...
		showT9({
			...
			inputLabel:i18n.t('Amount in SDR')
		}, ({value:amount, dialog})=>{
			let sompis = formatForMachine(amount||0);
			if(sompis > this.faucetFundsAvailable){
				let msg = i18n.t(`You can't request more than [n] SDR.`)
						.replace("[n]", SDR(this.faucetFundsAvailable||0))
				return dialog.setError(msg);//'
			}
			
			dialog.hide();

			this.getSedraFromFaucet(amount)
