...
class SedraSendDialogMobile extends SedraDialog{
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${totalAmount?html`<tr><td is="i18n-td">Total Amount</td><td> ${SDR(totalAmount)} SDR</td></tr>`:''}
			</table>
		</div>
		`
	}
	...
	showT9(e){
		...
		showT9({
			value,
			heading:input.label.replace("in SDR", ""),
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] SDR) is very large.');
...
SedraSendDialogMobile.define("sedra-send-dialog-mobile");
