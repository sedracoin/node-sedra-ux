import {
	html, css, FlowFormat, SedraWalletUI, dpc,
	baseUrl, SDR, renderPagination, buildPagination, paginationStyle,
...
} from './sedra-wallet-ui.js';
export {isMobile, dontInitiatedComponent};
export class SedraWalletMobile extends SedraWalletUI{
	...
	static get styles(){
		return [SedraWalletUI.styles, paginationStyle, swipeableStyle, css`
			:host{
			...
			.header{
				...
				padding:var(--sedra-wallet-header-padding, 5px 10px);
				margin:var(--sedra-wallet-header-margin, 0px);
			}
			...
			.pagination-box{
				padding:var(--sedra-pagination-box-padding, 10px 5px;);
			}

			.logo{
				width:30px;height:30px;/*background-color:#DDD*/
				display:var(--sedra-wallet-header-logo-diaplay, initial);
			}
			...
			.tabs-container{
				...
				border-top:2px solid var(--sedra-wallet-tab-border-top-color, var(--flow-primary-color));
				border-bottom:1px solid var(--sedra-wallet-tab-border-bottom-color, #DDD);
			}
			.tabs{
			...
			.tab.selected{
				border-bottom-color:var(--sedra-wallet-tab-active-border-color, var(--flow-primary-color));
			}
			...
			.address-input{
				...
				min-width:var(--sedra-wallet-address-input-min-width, 460px);
			...
			.send-scan-buttons a fa-icon{
				...
				background-color:var(--sedra-wallet-scan-button-bg, #3d4e58);
				--fa-icon-color:var(--sedra-wallet-scan-button-color, #FFF);
				box-shadow:var(--flow-box-shadow);
			}
			.send-scan-buttons .send-btn fa-icon{
				background-color:var(--sedra-wallet-send-button-bg, #fb7470);
				--fa-icon-color:var(--sedra-wallet-send-button-color, #FFF);
			}
			.send-scan-buttons .receive-btn fa-icon{
				background-color:var(--sedra-wallet-receive-button-bg, #60b686);
				--fa-icon-color:var(--sedra-wallet-receive-button-color, #FFF);
			}
			...
			.lock-screen{
				...
				position:absolute;
				z-index:1000000;
				background-color:var(--sedra-lock-screen-bg-color);
				top:0px;bottom:0px;
				left:0px;right:0px;
			}
		...
		<div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
			<div class="flow-swipeable-row">
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						<div class="badge" is="i18n-div">SEDRA WALLET</div>
						...
						<flow-expandable class="donation-info" no-info no-icon icon="-">
							...
							<p is="i18n-p">
								if you wish to further the development of the sedra ecosystem, we accept donations at the following addresses:
							</p>
							${
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Sedra Core SDR:</span> ${window.PWA_MODULES['@sedracoin/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Sedra Wallet Framework SDR:</span> ${window.PWA_MODULES['@sedracoin/wallet']}</div>
							<div class="badge"><span is="i18n-span">Sedra gRPC:</span> ${window.PWA_MODULES['@sedracoin/grpc']}</div>
							<div class="badge"><span is="i18n-span">Sedra gRPC Relay:</span> ${window.PWA_MODULES['@sedracoin/grpc-web']}</div>
							<div class="badge"><span is="i18n-span">Sedra UX SDR:</span> ${window.PWA_MODULES['@sedracoin/ux']}</div>
							<div class="badge"><span is="i18n-span">Flow UX SDR:</span> ${window.PWA_MODULES['@aspectron/flow-ux']}</div>
						</flow-expandable>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							<div class="margin-bottom"  is="i18n-div">SEDRA FAUCET</div>
							<div>${i18nFormat('Your IP is [n]', this.ip||"")}</div>
							<div class="margin">${i18nHTMLFormat('You have <b>[n] SDR</b> available.', SDR(this.faucetFundsAvailable||0) )}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${SDR(inUseUTXOs.satoshis||0)} SDR</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	renderLockScreen(){
		return html`
			<div class="lock-screen">
				<div><img class="big-logo" src="${baseUrl+'/resources/images/sedra.png'}"></div>
	...
	openAddressExplorer(address){
		if (!address)
			return
		let url = `https://explorer.sedracoin.com/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${ this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatSDR(total)+' SDR':''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatSDR(available)} SDR</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatSDR(pending)} SDR</span>
					</div>
				`}
            </div>
	...
	showReceiveDialog(){
		let address = this.receiveAddress||'sedratest:abc'
