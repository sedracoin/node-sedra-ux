...
import {
	html, css, SedraDialog, askForPassword, SDR,
	formatForMachine, getLocalWallet
} from './sedra-dialog.js';
const pass = "";
import {Wallet} from '@sedracoin/wallet-worker';

class SedraSendDialog extends SedraDialog{
	...
	static get styles(){
		return [SedraDialog.styles, 
		css`
			.container{
				max-height:var(--sedra-dialog-container-max-height, 670px);
			}
	...
	renderBody(){
		return html`
			<flow-input class="address full-width" outer-border
				label="${T(`Recipient Address (Must start with 'sedra' prefix)`)}"
				value="${this.address||''}"
				placeholder="">
			</flow-input>
			<div col>
				<flow-input class="amount full-width" outer-border
					label="${T('Amount in SDR')}" @keyup=${this.onAmountChange}
					value="${this.amount}">
				</flow-input>
				<div spacer></div>
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${dataFee?html`<tr><td is="i18n-td">Data Fee</td><td>${SDR(dataFee)} SDR</td></tr>`:''}
				${fee?html`<tr><td is="i18n-td">Total Fee</td><td>${SDR(fee)} SDR</td></tr>`:''}
				${totalAmount?html`<tr is="i18n-td"><td>Total Amount</td><td> ${SDR(totalAmount)} SDR</td></tr>`:''}
			</table>
		</div>
		`
	}
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] SDR) is too large.');
			msg = msg.replace('[n]', SDR(estimate.fee));
...
SedraSendDialog.define("sedra-send-dialog");
