...
import {helper, Storage, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@sedracoin/wallet-worker';
export const {Deferred, SDR, Decimal} = helper;
export {CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
const storage = new Storage({logLevel:'debug'});
let {baseUrl, debug, MAX_UTXOS_THRESHOLD=80, dontInitiatedComponent=false} = window.SedraConfig || {};
if(!baseUrl){
	baseUrl = (new URL("../", import.meta.url)).href;
	debug && console.log("SedraUX: baseUrl", baseUrl)
}
...
/*
const {Wallet, initSedraFramework, Storage} = require("sedra-wallet-worker");
let {Mnemonic} = Wallet;
...
export const {RPC} = require("sedra-grpc-node");
*/
