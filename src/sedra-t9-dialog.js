...
import {
	html, css, SedraDialog, SDR,
	formatForMachine, formatForHuman
} from './sedra-dialog.js';

class SedraT9Dialog extends SedraDialog{
	...
	static get styles(){
		return [SedraDialog.styles, css`
			.container{
				...
				max-height:var(--sedra-dialog-container-max-height, 600px);
			}
	...
	renderBody(){
		...
		let {inputLabel='Amount in SDR'} = this;
		return html`
	...
	open(args, callback){
		...
		this.inputLabel = args.inputLabel||i18n.t('Amount in SDR');
		this.show();
	}
    cancel(){
    	this.hide();
...
SedraT9Dialog.define("sedra-t9-dialog");
