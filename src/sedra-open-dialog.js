import {html, css, SedraDialog, i18n, T} from './sedra-dialog.js';
import {askForPassword, baseUrl} from "./wallet.js";
const pass = "";

class SedraOpenDialog extends SedraDialog{
	...
	static get styles(){
		return [SedraDialog.styles, css`
			.container{max-height:var(--sedra-dialog-container-max-height, 600px)}
			:host([mode="create"]) .container{max-height:var(--sedra-dialog-container-max-height, 500px)}
			:host([mode="init"]) .container{max-height:var(--sedra-dialog-container-max-height, 200px)}
			:host([mode="recover"]) .container{max-height:var(--sedra-dialog-container-max-height, 450px)}
			.buttons{justify-content:center;--sedra-dialog-buttons-width:100%;}
	...
	renderInitUI(){
		return html`
			<div class="sub-heading text-center" is="i18n-div">Welcome to Sedra Wallet</div>
		`
	}
	renderRecoverUI(){
		let rows = [0, 1, 2];
	...
	renderOpenUI(){
		...
		return html`
			${this.hideLogo?'': html`
			<div>
				<img class="big-logo" src="${baseUrl+'/resources/images/sedra.png'}" />
			</div>`}
...
SedraOpenDialog.define("sedra-open-dialog");
